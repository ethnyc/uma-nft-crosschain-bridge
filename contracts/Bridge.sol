...
contract Bridge is Testable {
    ...
    constructor(address finderAddress, address timerAddress)
    ...
    function deposit(Relay memory relay) public {
        relay.tokenURI = relay.isTokenURIIncluded
            ? IERC721Metadata(relay.nftContractAddress).tokenURI(relay.tokenId)
            : "";
        ...
        if (relay.nftContractAddress == address(_wrappedERC721)) {
            ...
            IERC721(relay.nftContractAddress).transferFrom(
                relay.from,
                address(this),
                relay.tokenId
            );
    ...
    function withdraw(Relay memory relay) public {
        ...
        require(
            block.timestamp >= relay.finalizedAt,
            "Bridge: relay is not yet finalized"
        );
        ...
        returns (OptimisticOracleV2Interface)
    {
        return
            OptimisticOracleV2Interface(
                _finder.getImplementationAddress("OptimisticOracleV2")
            );
    }
